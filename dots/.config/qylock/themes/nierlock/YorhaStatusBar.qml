...
Item {
    ...
    Process {
        id: fetchSysInfo
        command: ["sh", "-c", "OS=$(grep 'PRETTY_NAME' /etc/os-release | cut -d'\"' -f2); HOST=$(uname -n); KERN=$(uname -r); UP=$(uptime -p | sed -s 's/up //g'); PKGS=$(pacman -Q | wc -l); echo \"$OS|$HOST|$KERN|$UP|$PKGS\""]
        running: false
