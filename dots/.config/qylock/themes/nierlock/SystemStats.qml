...
Singleton {
    ...
    FileView {
        ...
        onLoaded: {
            const data = text();
            const matchTotal = data.match(/MemTotal: *(\d+)/);
            root.memTotal = matchTotal ? parseInt(matchTotal[1], 10) : 1;
            const matchAvail = data.match(/MemAvailable: *(\d+)/);
            root.memUsed = root.memTotal - (matchAvail ? parseInt(matchAvail[1], 10) : 0);
        }
    }
