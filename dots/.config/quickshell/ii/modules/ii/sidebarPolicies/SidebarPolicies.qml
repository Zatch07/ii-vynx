...
Scope { // Scope
    ...
    Process { // Dodge cursor away, pin, move cursor back
        ...
        function doIt2(output) {
            command = ["bash", "-c", "res=$(hyprctl dispatch 'hl.dsp.cursor.move({x=9999,y=9999})' 2>&1); [[ \"$res\" =~ \"error:\" ]] && hyprctl dispatch movecursor 9999 9999"];
            hook = () => {
                doIt3();
            }
            running = true;
        ...
        function doIt3(output) {
            root.pin = !root.pin;
            command = ["bash", "-c", `sleep 0.01; res=$(hyprctl dispatch 'hl.dsp.cursor.move({x=${cursorX},y=${cursorY}})' 2>&1); [[ "$res" =~ "error:" ]] && hyprctl dispatch movecursor ${cursorX} ${cursorY}`];
            hook = null
            running = true;
        }
