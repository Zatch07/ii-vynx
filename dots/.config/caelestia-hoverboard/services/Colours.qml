...
Singleton {
    ...
    function load(data: string, isPreview: bool): void {
        ...
        for (const key in json) {
            if (json.hasOwnProperty(key)) {
                ...
                if (colours.hasOwnProperty(m3Key))
                    colours[m3Key] = json[key];
            }
        }

        if (!isPreview) {
            currentLight = (colours.m3background.hslLightness >= 0.5);
            root.scheme = "Matugen";
        } else {
            previewLight = (colours.m3background.hslLightness >= 0.5);
        }
    }
    ...
    function reloadHyprRules(): void {
        // Disabled: Hyprland 0.55+ IPC doesn't support 'keyword layerrule' anymore.
        // The rule is now hardcoded in ~/.config/hypr/hyprland/rules.conf.
    }
    ...
    FileView {
        path: `${Paths.home}/.local/state/quickshell/user/generated/colors.json`
        watchChanges: true
